...
class Profiles extends UActiveRecord
{
	...
	public function rules()
	{
		...
		if (!$this->_rules) {
			...
			foreach ($model as $field) {
				...
				if ($field->varname=='hp')
				{
					$field_rule = array($field->varname, 'match', 'pattern' => '/^[+0-9_]+$/u');
					if ($field->error_message) $field_rule['message'] = "Incorrect symbols (0-9).";
					array_push($rules,$field_rule);
				}
	...
	{
		$monthy=(int)date('m',time());
		$dayn=(int)date('d',time());
		$nextmonth=intVal($monthy)+1;
		...
		for ($i=0;$i<count($temp);$i++)
		{
			...
			{
				list($year, $month, $day) = split('[/.-]', $temp[$i]->tgllahir);
				$month=(int)$month;
				$day=(int)$day;
				if ((($month==$monthy) && (($day-$dayn<=7)&&($day>=$dayn))) || (($month==$nextmonth) && ($dayn>23) && ($day<($dayn-23))))
				{
